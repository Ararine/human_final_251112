...
def create_curriculum(user_id, n_days, available_time):
    ...
    df = exercise.get_curriculum_by_id(user_id, start_date)
    if len(df)!=0:
        df = df[0]
...
def delete_curriculum_by_id(user_id):
    today_date = pd.Timestamp.today().strftime('%Y-%m-%d')
    df = exercise.delete_curriculum_by_id(user_id, today_date)
    return df
